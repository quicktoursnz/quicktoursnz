  ...
  return (
    <div className='max-w-6xl lg:mx-auto mx-4'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-14 mt-14">
          {tours?.map((t,index) => (
            <div onClick={()=> router.push(index < 6 || t.subPackages ? `/${t.slug}` : `/${t.slug}/book-now`)} key={index} className="flex flex-col shadow cursor-pointer p-2 rounded-xl">
              {/* Image placeholder */}
              ...
              <div className="flex items-center justify-between bg-main text-primary rounded-full px-5 py-2.5">
                <a href={index < 6 || t.subPackages ? `/${t.slug}` : `/${t.slug}/book-now`} className="flex items-center gap-1 text-sm font-medium text-primary-foreground hover:opacity-80 transition-opacity">
                  Book Now
